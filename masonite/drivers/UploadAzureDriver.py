...
class UploadAzureDriver(BaseUploadDriver, UploadContract):
    ...
    def store(self, fileitem, location=None): 
        ...
        block_blob_service = BlockBlobService(account_name=self.config.DRIVERS['azure']['name'], account_key=self.config.DRIVERS['azure']['secret'])
        ...
        block_blob_service.create_blob_from_path(self.config.DRIVERS['azure']['container'], filename, file_location)
        return filename
